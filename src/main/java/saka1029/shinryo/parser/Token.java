...
public class Token {
    ...
    /**
     * 兄弟ノードを識別する識別子。[a-zA-Z][a-zA-z0-9_+-]*
     * ピリオド(.)はパス区切りとして予約しているので使用しない。
     * 
     * HTMLにおけるid属性値:
     * 
     * HTML4.01:
     * id属性値は「アルファベット([A-Za-z])で開始し、任意の数のアルファベット、
     * 数字、([0-9])、ハイフン(-)、アンダースコア(_)、コロン(:)、ピリオド(.)のみで記述する必要がある。
     * 大文字/小文字を区別する(CS)。この名前は文書中で一意でなければならない(MUST)。」
     * 
     * HTML5:
     * HTML Living Standard (通称 HTML 5) ではid属性値、class属性値に対して
     * 空白文字を除く全ての文字を使用できます。
     * id属性値の「名前が一意でなければならない」のは HTML 4.01 と同様です。
     */
    public final String id;  // 兄弟ノードを識別する識別子。[a-zA-Z][a-zA-z0-9_+-]*
    ...
    public Token(TokenType type, String number, String header, String fileName, int pageNo, int lineNo, int indent, List<String> body) {
    	this.type = type;
    ...
    /**
     * Token orgを複製する。
     * ただしtype, number, header, bodyは変更する。
     */
